    ...
    for iter  = 2 : max_iter
        ...
        for ii = 1 : N
            ...
            for kk=1:n
                g0 = 0;
            for jj=1:m
                phi = unifrnd(0.1,0.7);%rand;%normrnd(0,1);%normrnd(-0.01,0.01);
                xi = 0;%normrnd(0,0.01);
                %delta = 1e-4;
                ekk = I(:,kk);
        ...
        for ii=1:N
            ...
            % calculate the gradient in the average point with FDM
             ...
             for kk=1:n
                g0 = 0;
            for jj=1:m
                phi = 1;%unifrnd(0.5,1);%rand;%normrnd(0,1);%normrnd(-0.01,0.01);
                %xi = normrnd(0,0.01);
                %delta = 1e-4;
                ekk = I(:,kk);
                f1 = fc(D((ii-1)*bs+1:ii*bs,:), v(:,ii)+delta*phi*ekk,xi);
...
for trial = 1:max_trial
   ...
   % semilogy(Opt_gap_dsgsq(:,trial),'--', 'color',[.4,0.8,1-m]);hold on;
    ...
    semilogy(Opt_gap_sgd(:,trial),'--', 'color',[.7,.7,.7-m]);hold on;
end
...
h5 = semilogy(mean(Opt_gap_sgd,2),'linewidth',linewidth, 'color', [0.7-m,0.7-m,0.7-m]);hold on;


%le = legend([h1,h2,h3,h4, h5],'PProx-PDA constant $\rho$','DSG $\rho =\mathcal{O} (1/r)$', 'DSG $\rho =\mathcal{O} (1/\sqrt{r})$','RGF', 'Steepest Grad');
le = legend([h1,h4,h5],'PZO-PDA','RGF (0th Order DSG)', 'SGD(0th Order)');
...
for trial = 1:max_trial
    ...
    semilogy(Constraint_vio_sgd(:,trial),'--', 'color',[.7,0.7,0.7-m]);hold on;
end
%plot the average
...
p5 = semilogy(mean(Constraint_vio_sgd,2), 'linewidth',linewidth, 'color', [.7-m,0.7-m,0.7-m]);


%le = legend([p1,p2,p3,p4,p5],'PProx-PDA constant $\rho$','DSG $\rho =\mathcal{O} (1/r)$', 'DSG $\rho =\mathcal{O} (1/\sqrt{r})$','RGF', 'Steepest Grad');
le = legend([p1,p4,p5],'PZO-PDA','RGF (0th Order DSG)','SGD (0th Order)');
xl = xlabel('Iteration Number','FontSize',fontsize);
