...
l_max = eigs(Lm,1);
    ...
    for iter  = 2 : max_iter
        ...
        for ii=1:N%ii=d1+1:d1+d2 % for d2 varibale project onto the l2 ball
            if norm(x_matrix(:,ii),inf)>1
                x_matrix(:,ii) = x_matrix(:,ii)/norm(x_matrix(:,ii),inf);
            end
        end
