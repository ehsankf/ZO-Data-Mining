...
def main():
    ...
    MGR.Add_Parameter('eta', MGR.parSet['alpha']/origImgs[0].size) #/np.sqrt(origImgs[0].size)
    MGR.Log_MetaData()
...
if __name__ == "__main__":
    ...
    parser.add_argument('-optimizer' , default='ZOSGD', help="choose from ZOSVRG and ZOSGD")
    ...
    #parser.add_argument('-alpha', type=float, default= 10, help="Optimizer's step size being (alpha)/(input image size)")
    parser.add_argument('-alpha', type=float, default= 15, help="Optimizer's step size being (alpha)/(input image size)")
    ...
    parser.add_argument('-rv_dist', default= 'CoordSphere', help="Choose from UnitSphere and UnitBall and CoordSphere")
