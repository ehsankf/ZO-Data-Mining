...
def main():
    ...
    MGR.Add_Parameter('eta', MGR.parSet['alpha']/np.sqrt(origImgs[0].size))#np.sqrt(origImgs[0].size)
    MGR.Log_MetaData()
...
if __name__ == "__main__":
    ...
    parser.add_argument('-optimizer' , default='ZOSAGA', help="choose from ZOSVRG and ZOSGD")
    ...
    #parser.add_argument('-alpha', type=float, default= 1.5, help="Optimizer's step size being (alpha)/(input image size)")
    #parser.add_argument('-alpha', type=float, default= 1, help="Optimizer's step size being (alpha)/(input image size)")
    parser.add_argument('-alpha', type=float, default= 0.7, help="Optimizer's step size being (alpha)/(input image size)")
    ...
    parser.add_argument('-lambda2', type=float, default=1e-5, help="Coefficeint on L2 regularizer")
    parser.add_argument('-lambda1', type=float, default=1e-5, help="Coefficeint on L1 regularizer")
    parser.add_argument('-nFunc', type=int, default=100, help="Number of images being attacked at once")
    ...
    parser.add_argument('-batch_size', type=int, default=30, help="Number of functions sampled for each iteration in the optmization steps")
    ...
    parser.add_argument('-rv_dist', default='CoordSphere', help="Choose from UnitSphere and UnitBall")
