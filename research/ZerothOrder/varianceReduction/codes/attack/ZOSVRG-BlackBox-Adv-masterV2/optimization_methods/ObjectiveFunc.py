...
class OBJFUNC:
    ...
    def evaluate(self, delImgAT, randBatchIdx, addQueryCount = True):
        ...
        self.Loss_L1 = self.lambda1 * np.mean(np.amax(np.amax(advImgs_Batched, 2), 1))
        self.Loss_L2x = self.lambda2 * self.imageSize * np.mean(np.square(advImgs)/2.0)
        self.Loss_Overall = self.Loss_L2 + self.const*self.Loss_Attack + self.Loss_L1 + self.Loss_L2x

        return self.Loss_Overall

    def gradient_estimation(self, delImgAT, mu, qx, qy, randBatchIdx = np.array([])):
