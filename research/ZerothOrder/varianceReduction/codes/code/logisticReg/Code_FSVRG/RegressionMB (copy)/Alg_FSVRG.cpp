...
mxArray* FSVRG(int nlhs, const mxArray *prhs[]) {
	...
	// Input variables
	double *w, *Xt, *y;
	double lambda, iters_outer, alpha, alpha1, alpha2, theta;		
    ...
    theta  = mxGetScalar(prhs[5]); // Momentum weight (constant)
	...
	for (k = 0; k < iters_outer; k++) {
		...
		for (i = 0; i < iters_inner; i++) {
            ...
            if (k == 0) {                    
                for (j = 0; j < d; j++) {
                    w_prev[j] += alpha1 * g_prev[j] + alpha2 * w_prev[j];
                    ww_prev[j] += w_prev[j];
                }
                if (i==iters_inner-1) {
                    for (j = 0; j < d; j++) {
                ...
                for (j = 0; j < d; j++) {
                    x_prev[j] += alpha * (g_tilda[j] - g_prev[j] - mu[j] - lambda * w_prev[j]);
        ...
        if (k > 0) {
            iters_inner = floor(1.65*iters_inner);
        }
    }
