...
batch_sizeArray = [1, 16, 64, 512, 1028];
innerloop = 5;
...
marker = {'r-.*', 'g-.o', 'b-.>', 'k-p', 'c-+', 'm-h', 'b-p', 'r-x', 'm-s', 'r-^', 'b-v', 'g-*', 'c-<', 'g->'};


method = '/PSCVRBatch/PSCVR';
scheme = 'ZO-PSVRG+';
...
%           squeeze(abs(hist1(i, j, 1:b:end)))];
    ...
    Batch =  BatchArray(1, i);
    batch_size = batch_sizeArray(1, j);
    Legend(index) = strcat(scheme, ' b = ', {' '}, num2str(batch_size));
...
switch k
    case 1
       xlabel('# of epochs');
       axis([0 900, 0.5E-3,aa])
    case  2 
       xlabel('Queries/1000');
%        axis([0 zoQB, 1E-5,aa]);
%        if(strcmp(par,'mnist'))
           axis([0 1e6, 0.5E-5,aa]);
%        end
       if(strcmp(par,'adult'))
           axis([0 1e6, 0.5E-5,aa]);
       end
...
legend boxoff
figname = strcat('results/', par, '/PSCVRBatch/', par, 'b', num2str(batch_sizeArray(1)), 'k', ...
    num2str(k),'.eps');
