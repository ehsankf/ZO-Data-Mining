	...
	for (k = 0; k < iters_outer; k++) {
		...
		if (evalf == true) {		
			...
			//hist[k] = compute_function_value(w_tilda, Xt, y, n, d, lambda, lambda1);
               ...
               if(k%1 == 0){
                   ...
                   //xrand +=k;
                   xrand +=1;
                   //printf("xrand %d num_Batches %d num %d", xrand, num_Batches, xrand % num_Batches);
                   Batch_index = xrand % num_Batches; //dis(gen) % num_Batches;;//rand() % num_Batches;
                   //printf("\t Batch_index %d\n",  Batch_index);
		   left_Batch = Batch_index * Batch_size;
		   right_Batch = (Batch_index + 1) * Batch_size;
                   if (Batch_index == num_Batches-1) 
		       right_Batch = n; 
                   size_Batch = right_Batch - left_Batch;
                   }
		...
		for (i = 0; i < iters_inner; i++) {
			...
			if (train_index == num_batches_train-1) 
					right = n;
                        int size = right - left;
                        //printf("Itr %d BatchIndex %d\n", k, train_index);
			//long idx = dis(gen);
            
			// Compute gradient of last inner iter
			//compute_partial_gradient(Xt, w_prev, y, g_prev, n, d, lambda, idx);
			compute_zeroth_partial_gradientRSG(Xt, w_prev, y, g_prev, n, d, lambda, lambda1, left, right, 1);
                        //printf("left %d right %d\n", left, right);

			// Compute the gradient of last outer iter
			//compute_partial_gradient(Xt, w_tilda, y, g_tilda, n, d, lambda, idx);
                        compute_zeroth_partial_gradientRSG(Xt, w_tilda, y, g_tilda, n, d, lambda, lambda1, left, right, 1);
