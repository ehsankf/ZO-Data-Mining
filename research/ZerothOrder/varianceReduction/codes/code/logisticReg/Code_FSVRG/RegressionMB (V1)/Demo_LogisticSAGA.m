...
for k=1:3
 ...
 for i=1:nB
  for j=1:mb
    minval =  abs(hist1(i, j, end))-1e-10;
    Xres = [squeeze(hist1(end, end, 1:b:end)), squeeze(floor(zerothhist(i, j, 1:b:end)/1000)), squeeze(floor(timehist(i, j, 1:b:end)))/1000, ...
          squeeze(abs(hist1(i, j, 1:b:end))- minval)];
    ...
    name = strcat('results/',par,'/SAGA/SAGA', 'B', num2str(i), 'b', num2str(j));
    dlmwrite(name, Xres, 'delimiter', '\t', 'precision', 32);
    Xres = dlmread(name);
    semilogy(Xres(:,k) - Xres(1,k), Xres(:,4), ...
...
legend(Legend)
figname = strcat('results/',par,'/SAGA/SAGA', 'B', num2str(i), 'b',...
    num2str(j));
