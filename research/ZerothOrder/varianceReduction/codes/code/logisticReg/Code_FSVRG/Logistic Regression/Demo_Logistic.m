...
% eta = 1.00 / (10 * Lmax);
% batch_size = 1;
% num_batches_train = n / batch_size;
% tic;
% hist1 = Alg_SAGAI(w, full(X), y, lambda, lambda1, eta, outer_loops);
% time_SAGA = toc;
...
eta = 1.00 / (10 * Lmax);
tic;
hist2 = Alg_SVRG(w, full(X), y, lambda, lambda1, eta, outer_loops);
time_SVRG = toc;
...
aa = max(max([hist2(:,2),hist2(:,2)]))-minval; b = 1;
h = figure(11)
set(gcf,'position',[200,100,386,269]) 
semilogy(hist2(1:b:end,1), abs(hist2(1:b:end,2) - minval),'g-.o','linewidth',1.6,'markersize',4.5);
