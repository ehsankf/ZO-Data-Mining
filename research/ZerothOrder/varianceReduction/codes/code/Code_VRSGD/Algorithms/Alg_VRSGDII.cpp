...
mxArray* SVRG(int nlhs, const mxArray *prhs[]) {
	...
	// Input variables
	double *w, *Xt, *y;
	double lambda, iters_outer, alpha, alpha1, beta, tau;	
	...
	for (k = 0; k < iters_outer; k++) {
        ...
        tau = 2/(k+2); tau = maxi((double)0.20, (double)tau); 
        alpha1 = alpha/tau;
        beta = 1/(1 + lambda*alpha1);
		...
		for (i = 0; i < iters_inner; i++) {
			...
			// Update w_prev
            for (j = 0; j < d; j ++) {
                w_prev[j]  += alpha1 * (g_tilda[j] - g_prev[j] - mu[j]); 
                w_prev[j]  = beta * w_prev[j];
                ww_prev[j] += w_prev[j];
            }
		}
