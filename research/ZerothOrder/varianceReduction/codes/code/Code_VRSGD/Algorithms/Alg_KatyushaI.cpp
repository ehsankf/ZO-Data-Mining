...
mxArray* Katyusha(int nlhs, const mxArray *prhs[]) {
    ...
    // Parameters
    Lmax = 3*Lmax;
    tau  = mini(0.5, sqrt(2*n*lambda/Lmax));
    alpha = 1.00 / (1.0*tau*Lmax); 
    tau1  = 1 + lambda*alpha;
	...
	for (k = 0; k < iters_outer; k++) {
		...
		for (i = 0; i < iters_inner; i++) {
            ...
            for (j = 0; j < d; j ++) {
                g_svrg[j] = g_prev[j] - g_tilda[j] + mu[j] + lambda * w_prev[j];  
                x_prev[j] -= alpha * g_svrg[j]; 
                y_prev[j] = w_prev[j] - (1/Lmax) * g_svrg[j]; 
