...
/// SVRG runs the SVRG algorithm for solving L2-norm regularized 
/// ridge regression on dense data provided
...
mxArray* SVRG(int nlhs, const mxArray *prhs[]) {
	...
	// Input variables
	double *w, *Xt, *y;
	double lambda, iters_outer, alpha, beta;	
	...
	for (k = 0; k < iters_outer; k++) {
		...
		compute_full_gradient(Xt, w_tilda, y, mu, n, d, 0);
		// Initiate ww_prev
		for (j = 0; j < d; j ++) {
            ww_prev[j] = 0;
		}
        
        beta = 1/(1 + lambda*alpha);
		...
		for (i = 0; i < iters_inner; i++) {
			...
			compute_partial_gradient(Xt, w_prev, y, g_prev, n, d, 0, idx);
			
			// Compute the gradient of last outer iter
			compute_partial_gradient(Xt, w_tilda, y, g_tilda, n, d, 0, idx);

			// Update w_prev
            for (j = 0; j < d; j ++) {
                g_svrg[j] = alpha * (g_prev[j] - g_tilda[j] + mu[j]); 
                w_prev[j] = beta * (w_prev[j] - g_svrg[j]);    
            }
