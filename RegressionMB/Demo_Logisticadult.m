...
lambda1 = 10^(-4); % Regularization parameter
...
for i=1:nB
  for j=1:mb  
    ...
    tic;
%     
    xxoutput= Alg_SVRG(w, full(X), y, lambda, lambda1, eta, outer_loops, batch_size, Batch, innerloop);
...
minval =  min(min(abs(hist1(1:end-1, 1:end-1, 1:end))));
