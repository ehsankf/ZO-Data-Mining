...
switch par
    case 'mnist'
       eta = 0.1/(Lmax * d(1)^0.5);
       outer_loops = 300;
    case  'Covtype' 
       eta = 1 /(2 * Lmax * d(1)^0.5);
       outer_loops = 300;
    otherwise
       eta = 1 / (5 * Lmax * d(1)^0.5);
       %eta = 1.00 / (d(1)^0.5);
       outer_loops = 300;
end
...
for i=1:nB
  for j=1:mb 
    ...
    tic;
    xxoutput= Alg_PSCVRRGE(w, full(X), y, lambda, lambda1, eta, outer_loops, batch_size, Batch, innerloop);
...
minval =  min(min(min(abs(hist1(1:end-1, 1:end-1, 1:end)))));
