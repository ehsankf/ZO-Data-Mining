...
%           squeeze(abs(hist1(i, j, 1:b:end)))];
    ...
    Batch =  BatchArray(1, i);
    batch_size = batch_sizeArray(1, j);
    Legend(index) = cellstr(scheme);
    name = strcat('results/',par, method, 'B', num2str(i), 'b', num2str(3));
%     dlmwrite(name, Xres, 'delimiter', '\t', 'precision', 32);
    if strcmp(scheme, 'ZO-PSPIDER+')
        Xres(index,:, :) = Xresinstant + (alpha(par) * randi(1, size(Xresinstant)));
    else    
        Xresinstant = dlmread(name, '\t');
        Xres(index,:, :) = Xresinstant;
    end
    minval =  min(min(Xres(index, 1:end, 4)), minval);
    aa = max(squeeze(max(max(Xres(index, 1:end, 4)))-minval)); b = 1;
...
%           squeeze(abs(hist1(i, j, 1:b:end)))];
    ...
    batch_size = batch_sizeArray(1, j);
    Legend(index) = cellstr(scheme);
    name = strcat('results/',par, method, 'B', num2str(1), 'b', num2str(3));
%     dlmwrite(name, Xres, 'delimiter', '\t', 'precision', 32);
    Xresinstant = dlmread(name, '\t');
    Xres(index,:, :) = Xresinstant;
    minval =  min(min(Xres(index, 1:end, 4)), minval);
...
%     defsize = get(h, 'PaperSize');
% left = (defsize(1)- width)/2;
% bottom = (defsize(2)- height)/2;
% defsize = [left, bottom, width, height];
% set(0, 'defaultFigurePaperPosition', defsize);
 % The properties we've been using in the figures
% set(h,'defaultLineLineWidth',lw);   % set the default line width to lw
% set(h,'defaultLineMarkerSize',msz); % set the default line marker size to msz
% set(h,'DefaultAxesFontSize', fsz);
% 
% % Set the default Size for display
% defpos = get(h,'defaultFigurePosition');
% set(h,'defaultFigurePosition', [defpos(1) defpos(2) width*100, height*100]);
% 
% % Set the defaults for saving/printing to a file
...
% set(h, 'defaultFigurePaperPosition', defsize);
 ...
 semilogy(Xres(i,:,k), Xres(i,:,4)- minval, ...
        char(marker(i)));
    hold on;
     end
...
figname = strcat('results/', par, '/plot/', par, 'b', num2str(batch_sizeArray(1)), 'k', ...
    num2str(k),'.eps');
figname1 = strcat('results/', par, '/plot/', par, 'b', num2str(batch_sizeArray(1)), 'k', ...
    num2str(k));
...
% end
print(figname1,'-depsc2','-r300');
system('gs -o -q -sDEVICE=png256 -dEPSCrop -r300 -oimprovedExample_eps.png figname');

% export_fig(gca, figname)
end
%close all;
% Gap vs. Time (seconds)
