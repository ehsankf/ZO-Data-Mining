...
all_pathes = ['mnist', 'ZOSVRG1', 'ZOPSVRG', 'ZOPSPIDER', 'ZOPSVRGRSGE', 'ZOSAGA1', 'ZOSGD3']
...
for row in range(nrows):
    ...
    for col in range(ncols):
        ...
        if row == 0 and col == ncols - 2:
            axis.annotate('Global\nPerturbation\n' + r'($\delta$)', (0.5, 0.5),
                          ha='center', va='center', fontsize=8,
                          color='black')
            continue
  
        if row == 0 and col == ncols - 1:
            axis.annotate('Perturbation\nNorm\n' + r'($\|\|\delta\|\|$)', (0.5, 0.5),
                         ha='center', va='center', fontsize=8,
                         color='black')
            continue
        ...
        """
        if col == 0:
            axis.annotate(algos[row], (0.0, 0.5),
        ...
        if col == ncols - 1:
            ...
            axis.annotate(f'{l2_norm_delta:.2f}', (0.5, 0.5),
                          ha='center', va='center', fontsize=18,
                          color='black')
            continue
        ...
        perturbed_image = np.around((perturbed_image + 0.5)*255)
        perturbed_image = perturbed_image.reshape(28,28).astype(np.uint8).squeeze()
        axis.imshow(perturbed_image, interpolation='nearest', cmap="gray")
...
# plt.show()
plt.pause(1.5)
plt.savefig("{}.png".format("mnist"), bbox_inches='tight')
plt.close()
