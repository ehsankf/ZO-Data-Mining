...
Legend = {'ZO-ProxSVRG', 'ZO-PSVRG+', 'ZO-PSPIDER+', 'ZO-ProxSVRG (RandSGE)', 'ZO-PSVRG+ (RandSGE)', ...
'ZO-ProxSAGA', 'ZO-ProxSGD'}
Dir = {'ZOSVRG/', 'ZOPSVRG/', 'ZOPSPIDER/', 'ZOSVRGRSGE', 'ZOPSVRGRSGE/', 'ZOSAGA/', 'ZOSGD/'}
% Legend = {'ZO-PSVRG+', 'ZO-PSVRG+ (RandSGE)', 'ZO-SAGA', 'ZO-ProxSGD'}
...
for i=1:nB
    ...
    dirpath
    Xres = rmmissing(readtable(dirpath, 'Range','18:1000'));
        
    ms = size(Xres, 1);
    %     Xplot = [Xres{20:ms,6}/1000, Xres{20:ms,12}];
    if (strcmp('ZOSGD/', Dir(i)))
%        ms = 30000
       Xplot = [Xres{20:ms,5}/1000, Xres{20:ms,11}]; 
       Xplot1 = [Xres{20:10:ms,3}/10, Xres{20:10:ms,11}];
    else
        Xplot = [Xres{20:ms,6}/1000, Xres{20:ms,12}];
