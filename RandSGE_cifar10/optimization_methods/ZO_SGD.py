...
def ZOSGD(delImgAT_Init, MGR, objfunc):
    ...
    for T_idx in range(T):
        ...
        F_estimate  = objfunc.gradient_estimation(curret_delImgAT, curret_delImgAT, MGR.parSet['mu'],MGR.parSet['qx'], MGR.parSet['qy'], randBatchIdx)

        F_estimate += MGR.parSet['lambda2'] * F_estimate
        curret_delImgAT = curret_delImgAT - MGR.parSet['eta'] * F_estimate
        for j in range(0, curret_delImgAT.shape[0]):
                 for i in range(0, curret_delImgAT.shape[0]):
                     if((curret_delImgAT[i][j][0] > param)):
                           curret_delImgAT[i,j,0] -= param
                     elif(curret_delImgAT[i,j,0] < -param):
                           curret_delImgAT[i,j,0] += param
                     else:
                            curret_delImgAT[i,j,0] = 0;
