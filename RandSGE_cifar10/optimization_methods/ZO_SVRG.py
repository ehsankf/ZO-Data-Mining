...
def ZOSVRG(delImgAT_Init, MGR, objfunc):
    ...
    for S_idx in range(1, MGR.parSet['nStage']+1):
        ...
        for k in range(0, MGR.parSet['M']):
            ...
            v_k_S  = objfunc.gradient_estimation(delImgAT_k_S, delImgAT_0_S, MGR.parSet['mu'], MGR.parSet['qx'], MGR.parSet['qy'], randBatchIdx)
            if MGR.parSet['qx'] > 1:
                v_k_S -= objfunc.gradient_estimation(delImgAT_0_S, delImgAT_0_S, MGR.parSet['mu'], MGR.parSet['qx'], MGR.parSet['qy'], randBatchIdx)
            v_k_S += g_S
            v_k_S += MGR.parSet['lambda2'] * delImgAT_k_S
            delImgAT_kp1_S = delImgAT_k_S - MGR.parSet['eta'] * v_k_S
            # print('norm:', np.linalg.norm(g_S.flatten(), ord=2))
