...
def ZOSPIDER(delImgAT_Init, MGR, objfunc):
    ...
    param = MGR.parSet['eta'] * MGR.parSet['lambda1']
    for S_idx in range(1, MGR.parSet['nStage']+1):
        ...
        #print(randBatchBdx)
        #print("\n")
        g_S = objfunc.gradient_estimation(delImgAT_M_Sm1, delImgAT_M_Sm1, MGR.parSet['mu'], 28, 28, randBatchBdx)
        delImgAT_0_S = delImgAT_M_Sm1

        objfunc.evaluate(delImgAT_0_S, np.array([]), False)

        delImgAT_kp1_S = delImgAT_0_S
        delImgAT_k_1_S = delImgAT_kp1_S
        for k in range(0, MGR.parSet['M']):
            ...
            v_k_S  = objfunc.gradient_estimation(delImgAT_k_S, delImgAT_k_1_S, MGR.parSet['mu'], MGR.parSet['qx'], MGR.parSet['qy'], randBatchIdx)
            if MGR.parSet['qx'] > 1:
                v_k_S -= objfunc.gradient_estimation(delImgAT_k_1_S, delImgAT_k_1_S, MGR.parSet['mu'], MGR.parSet['qx'], MGR.parSet['qy'], randBatchIdx)
            delImgAT_k_1_S = delImgAT_k_S
            v_k_S += g_S
            v_k_S += MGR.parSet['lambda2'] * delImgAT_k_S
            delImgAT_kp1_S = delImgAT_k_S - MGR.parSet['eta'] * v_k_S
            g_S = v_k_S
            
            for j in range(0, delImgAT_Init.shape[0]):
                 for i in range(0, delImgAT_Init.shape[0]):
                     if((delImgAT_kp1_S[i][j][0] > param)):
                           delImgAT_kp1_S[i,j,0] -= param
                     elif(delImgAT_kp1_S[i,j,0] < -param):
                           delImgAT_kp1_S[i,j,0] += param
                     else:
                            delImgAT_kp1_S[i,j,0] = 0;
            ...
            MGR.logHandler.write(' Loss_L1: ' + str(objfunc.Loss_L1))
            MGR.logHandler.write(' Loss_L2: ' + str(objfunc.Loss_L2x))
