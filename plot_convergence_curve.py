...
def plot_train_eval_conv_curve(x, results, dir_name, x_ticks, y_ticks, y_ticks_label, index=0):
    ...
    for i, y in enumerate(results):
        mark = [np.argmin(np.abs(np.array(x[i]) - t)) for t in x_ticks[i if i - len(x_ticks) < 0 else i - len(x_ticks)]]
        plt.plot(x[i][0:], y[0:], colors[i], label=legend[i], linewidth=5.0, alpha=0.9, mfc='k', mec='k', marker='s', markersize=15, markevery=mark)

    x_ticks = [i for pair in zip(x_ticks[1], x_ticks[0]) for i in pair]
    x_ticks = x_ticks[1:]
    plt.yticks(ticks=y_ticks, labels=y_ticks_label, fontsize=25, weight='bold')
    plt.xticks(ticks=x_ticks, labels=x_ticks_label, fontsize=25, weight='bold')       
    plt.legend(loc='best', frameon=False, fontsize=25, handler_map={plt.Line2D:HandlerLine2D(update_func=update_prop)})
    ...
    plt.gcf().set_size_inches(21, plt.gcf().get_size_inches()[1])
    plt.tight_layout()
    plt.savefig(os.path.join(dir_name,
                          "{}.png".format(y_names[0])))
    #plt.savefig(y_names[index] + 'attack.png')
...
y_ticks_label = {}
x_ticks_label = [r'$1$', r'$\,1/\epsilon^{1/2}\,$', r'$n^{1/2}\,$', r'$\,\epsilon^{2/3}\,$', r'$n^{2/3}$', r'$1/\epsilon$', r'$n$']
...
batches = []
methods = ['svrg', 'spider']
y_ticks = {method: [] for method in methods}
for method in methods:
    for Ba in [n, 1./eps]:
        ...
        y_ticks[method].append(order_benches[method](Ba))
...
for method in methods:
    y_ticks[method] = [[1./norm_factor * elm for elm in y_ticks[method][i]]  for i in range(len(y_ticks[method]))]
    y_ticks[method] = [i for pair in y_ticks[method] for i in pair]
    y_ticks_label[method] = [i for pair in y_ticks_label[method] for i in pair]
#y_ticks_label_svrg = [r'$n^{3/4}/\epsilon$', r'$n^{3/4}/\epsilon$', r'$1/\epsilon^{5/3}$', r'$n^{2/3}/\epsilon$', r'$1/\epsilon^2$', r'$/\epsilon$']
legend = [leg for mth in methods for leg in legend[mth]]

plot_train_eval_conv_curve(batches, results, './', x_ticks, y_ticks['svrg'] + y_ticks['spider'], y_ticks_label['svrg'] + y_ticks_label['spider'], index=0)
