...
def plot_train_eval_conv_curve(x, results, dir_name, x_ticks, y_ticks, y_ticks_label, index=0):
    ...
    for i, y in enumerate(results):
        ...
        plt.plot(x[i][0:], y[0:], colors[i], label=legend[i], alpha=0.9, mfc='k', marker='o', markevery=mark)
    ...
    plt.gcf().set_size_inches(23, plt.gcf().get_size_inches()[1])
    plt.savefig(os.path.join(dir_name,
                          "{}.png".format(y_names[0])))
    #plt.savefig(y_names[index] + 'attack.png')
...
colors = ['r-', 'g+--', 'b+-', 'b+-.', 'c+-.']  
legend = [r'ZO-PSVRG+ $s_n = n$', 'ZO-PSPIDER+']
y_names = ['SZO Complexity']

n, eps = 10., 0.12 # 2., 0.55
...
y_ticks_label_svrg = [[r'$n^{3/4}/\epsilon$', r'$n^{2/3}/\epsilon$', r'$n/\epsilon$'], [r'$1/\epsilon^{7/4}$', r'$1/\epsilon^{5/3}$', r'$1/\epsilon^2$']] 
results = []
y_ticks_svrg = []
batches_svrg = []
for Ba in [n, 1./eps]:
    batches = list(np.linspace(1.0, (Ba ** (1/2.))-0.0001, 500)) + [Ba ** (1/2.)] + list(np.linspace((Ba ** (1/2.)) + 0.01, Ba ** (2./3), 500))
    Batches = [Ba] * 500 + [Ba] + [Ba] * 500
    # define B's and b's  
    ...
    order_svrg = [conv_order_svrg(B, b) for (B, b) in zip(Batches, batches)]
    # y_ticks_svrg = [x / order_svrg[-1] for x in [n/eps ** (3./4), n ** (3./4) / eps, n / eps ** (2./3), n ** (2./3) / eps, 1. / eps ** 2, n*1./eps]]
    y_ticks_svrg.append([x / order_svrg[-1] for x in [Ba ** (3./4) / eps, Ba ** (2./3) / eps, Ba * 1./eps]])
    # normalize batches and orders
    batches_svrg.append([scale_f * x for x in batches])
    order_svrg = [x / order_svrg[-1] for x in order_svrg]
    results.append(order_svrg)

#y_ticks_label_svrg = [r'$n^{3/4}/\epsilon$', r'$n^{3/4}/\epsilon$', r'$1/\epsilon^{5/3}$', r'$n^{2/3}/\epsilon$', r'$1/\epsilon^2$', r'$/\epsilon$']

y_ticks_svrg = [i for pair in zip(*y_ticks_svrg) for i in pair]
y_ticks_label_svrg = [i for pair in zip(*y_ticks_label_svrg) for i in pair]
plot_train_eval_conv_curve(batches_svrg, results, './', x_ticks, y_ticks_svrg, y_ticks_label_svrg, index=0)
